...
public class Main {
    ...
    public static void medic( int index) {
        if (heroesHealth[3] != 0) {
            if (heroesHealth[index] != 0) {
                if (heroesHealth[index] < 100) {
                    System.out.println("медик вылечит " + heroesSuperAbilities[index] + " " + heroesHealth[index]);
                    heroesHealth[index] = heroesHealth[index] + 20;
                    System.out.println("медик вылечил " + heroesSuperAbilities[index] +
                            " его здоровье теперь = " + heroesHealth[index]);
